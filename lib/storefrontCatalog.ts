...
function mergeProduct(base: SiteProduct, stored?: StoredProduct): SiteProduct | null {
  ...
  const customShortDescription = stored.shortDescription && !/for CHEERDMOTO retail, dealer and service sales/i.test(stored.shortDescription);
  const customDescription = stored.fullDescription && !/connected to the Cheerdmoto admin system/i.test(stored.fullDescription);
  ...
  return {
    ...
    category: stored.categoryName || base.category,
    ...
    shortDescription: customShortDescription ? stored.shortDescription : base.shortDescription,
    description: customDescription ? stored.fullDescription : base.description,
    keyFeatures: customFeatures ? stored.keyFeatures : base.keyFeatures,
    specifications: customFeatures && stored.specifications?.length
      ? stored.specifications.map((item) => ({ ...item, group: "Product details" }))
      : base.specifications,
    ...
    sku: stored.sku || base.sku,
    inventory,
    stockStatus: inventory === 0 ? "out_of_stock" : base.stockStatus === "preorder" ? "preorder" : "in_stock",
    shippingInfo: customShipping ? stored.shippingInfo : base.shippingInfo,
