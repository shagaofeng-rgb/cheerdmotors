...
export const ADMIN_COOKIE_NAME = "cheerdmotors_admin_session";
...
export function verifyAdminCredentials(email: string, password: string) {
  const adminEmail = (process.env.ADMIN_EMAIL || "admin@cheerdmotors.com").trim().toLowerCase();
  ...
  return process.env.NODE_ENV !== "production" && password === "cheerd-admin-demo";
}
...
export async function requireAdminApiSession() {
  const session = await getAdminSession();
  if (!session) return { session: null, response: Response.json({ message: "Admin login required" }, { status: 401 }) };
