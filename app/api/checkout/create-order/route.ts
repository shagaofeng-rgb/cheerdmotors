...
export async function POST(request: Request) {
  ...
  const customer = { name: str(payload.name), email: str(payload.email), phone: str(payload.phone), company: str(payload.company), country: str(payload.country), address: str(payload.address, 500), message: str(payload.message, 1000) };
  if (!customer.name || !customer.country || !validEmail(customer.email) || !customer.address) return Response.json({ message: "Name, valid email, country and shipping address are required." }, { status: 400 });
  let order;
  try {
  ...
  try {
    ...
    // The order is durable already; analytics must not turn a successful order into a retry and duplicate it.
  }
