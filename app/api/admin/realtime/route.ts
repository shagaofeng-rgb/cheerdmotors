...
export async function GET() {
  ...
  const state = { orders: orders.length, events: events.length, leads: leads.length, products: backend.metrics.products, posts: backend.metrics.posts, latestOrder: latest(orders.map((order) => order.updatedAt || order.createdAt)), latestEvent: latest(events.map((event) => event.timestamp)), latestLead: latest(leads.map((lead) => lead.lastActiveTime)) };
