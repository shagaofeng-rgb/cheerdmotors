...
export async function POST(request: Request) {
  ...
  await writeAdminStore((store) => ({
    ...store,
    categories: [...store.categories, { id: `cat-${Date.now()}`, name: text(formData, "name", 160), slug: text(formData, "slug", 120), description: "", coverImage: text(formData, "coverImage", 260), seoTitle: text(formData, "seoTitle", 180), seoDescription: "", sortOrder: store.categories.length + 1, status: "published", parentId: "", updatedAt: now }],
  }));
