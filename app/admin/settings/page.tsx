...
export default async function AdminSettingsPage() {
  ...
  return (
    <AdminShell active="settings">
      ...
      <section className="admin-panel"><div><p className="eyebrow">环境变量检查</p><h2>生产环境必须配置</h2></div><dl className="admin-config-list"><div><dt>后台登录</dt><dd>ADMIN_EMAIL, ADMIN_PASSWORD_HASH 或 ADMIN_PASSWORD, ADMIN_JWT_SECRET</dd></div><div><dt>订单存储</dt><dd>{stableStore ? "已连接 KV / Upstash Redis，订单和会员数据会稳定保存。" : "建议配置 KV_REST_API_URL + KV_REST_API_TOKEN，或 UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN。"}</dd></div><div><dt>邮件</dt><dd>SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, SMTP_FROM</dd></div><div><dt>支付</dt><dd>后续按 Cheerdmoto 收款通道接入；当前为人工报价/订单记录模式。</dd></div></dl></section>
    </AdminShell>
  );
}
