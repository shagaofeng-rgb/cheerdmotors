...
function usd(cents: number) {
  return cents ? `USD ${(cents / 100).toLocaleString()}` : "-";
}
...
export default async function AdminProductsPage() {
  ...
  return (
    <AdminShell active="products">
      <div className="admin-title"><p className="eyebrow">Product CMS</p><h1>产品管理</h1><p>按商城站逻辑管理产品草稿、发布状态、SKU、库存、价格、媒体图和 SEO 字段。</p></div>
      <section className="admin-panel admin-guidance-panel">
        <article><span>1</span><strong>保存草稿</strong><p>先录入产品资料、图片、价格、库存和 SEO。</p></article>
        <article><span>2</span><strong>发布产品</strong><p>状态改为已发布后，作为前台展示和商城下单数据。</p></article>
        <article><span>3</span><strong>下架/归档</strong><p>旧产品保留后台数据，但不建议继续展示给客户。</p></article>
      </section>
      <section className="admin-panel">
        <div><p className="eyebrow">快速新增</p><h2>新增产品</h2></div>
        <form className="admin-form-grid admin-form-wide" action="/api/admin/products" method="post">
          <input name="name" placeholder="产品名称，例如 XCEED" required />
          ...
          <input name="coverImage" placeholder="/volt-lab/products/xceed_transparent.png" />
          <input name="weightDimension" placeholder="重量/尺寸/包装" />
          <textarea name="galleryImages" placeholder="产品相册图片路径，每行一个，或用英文逗号分隔" />
          <textarea name="shortDescription" placeholder="产品短描述" />
          <textarea name="fullDescription" placeholder="产品详细描述、应用场景、包装说明、交付说明" />
          <textarea name="shippingInfo" placeholder="物流说明" />
          ...
          <label className="admin-check"><input type="checkbox" name="allowCart" defaultChecked /> 允许购物/询盘</label>
      ...
      <section className="admin-panel">
        <div><p className="eyebrow">产品数据库</p><h2>{products.length} 条产品记录</h2></div>
        <div className="admin-table-wrap">
          <table>
            <thead><tr><th>产品</th><th>分类</th><th>价格</th><th>库存</th><th>媒体</th><th>状态</th><th>SEO</th></tr></thead>
            <tbody>
              {products.length ? products.map((product) => (
                <tr key={product.id}>
                  <td><strong>{product.name}</strong><br /><small>{product.slug} | {product.sku}</small></td>
                  <td>{product.categoryName}</td>
                  <td><strong>{usd(product.salePriceCents || product.priceCents)}</strong><br /><small>原价 {usd(product.priceCents)}</small></td>
