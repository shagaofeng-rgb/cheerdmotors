...
export default async function AdminNewsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const posts = (await listAdminPosts("news")).filter((post) => !query.search || `${post.title} ${post.slug} ${post.source} ${post.category}`.toLowerCase().includes(query.search.toLowerCase()));
