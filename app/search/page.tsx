  ...
  return (
    <main className="content-site">
      ...
      <section className="search-results">
        <h2>Products</h2>{productResults.length ? productResults.map((product) => <Link href={`/products/${product.slug}`} key={product.slug}>{product.name}<span>{product.category}</span></Link>) : <p>No product matches yet.</p>}
