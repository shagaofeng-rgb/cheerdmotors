  ...
  return (
    <main className="content-site">
      ...
      <section className="search-results">
        ...
        <h2>{q ? "Content" : "Latest content"}</h2>{postResults.length ? postResults.map((post) => <Link href={postPath(post)} key={post.id}>{post.title}<span>{post.type} · {post.publishDate}</span></Link>) : <p>No content matches yet.</p>}
      </section>
    </main>
  );
}
