...
export default function AdminTimeFilter({ action, range, start, end, label, summary }: { action: string; range: string; start: string; end: string; label: string; summary: string }) {
    ...
    <form ref={formRef} className="admin-time-filter" action={action} method="get" aria-label={`${label}时间筛选`}>
      ...
      <div className="admin-time-presets" role="group" aria-label="快捷时间范围">
        {quickRanges.map((item) => (
          <button className={selectedRange === item.value ? "active" : ""} key={item.value} type="button" onClick={() => applyQuickRange(item.value)}>
            {item.label}
          </button>
        ))}
      </div>
      <label>
        ...
        <select
          ...
          onChange={(event) => {
            ...
            if (event.target.value !== "custom") window.setTimeout(() => formRef.current?.requestSubmit(), 0);
          }}
        >
