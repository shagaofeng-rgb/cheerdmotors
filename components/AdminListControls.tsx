...
export function AdminListControls({ action, query, countries = [], channels = [], classifications = [] }: { action: string; query: AdminListQuery; countries?: string[]; channels?: string[]; classifications?: string[] }) {
  return (
    <form className="admin-list-controls" action={action} method="get">
      ...
      <select name="country" defaultValue={query.country}><option value="">全部国家/地区</option>{countries.map((value) => <option key={value}>{value}</option>)}</select>
      <select name="channel" defaultValue={query.channel}><option value="">全部渠道</option>{channels.map((value) => <option key={value}>{value}</option>)}</select>
